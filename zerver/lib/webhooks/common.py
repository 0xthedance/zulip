...
def guess_zulip_user_from_external_account(
    realm: Realm, external_username: str, external_account_field_name: str
) -> UserProfile | None:
    try:
    ...
    return matching_zulip_users[0].user_profile if len(matching_zulip_users) == 1 else None
