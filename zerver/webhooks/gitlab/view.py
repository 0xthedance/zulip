...
def fixture_to_headers(fixture_name: str) -> dict[str, str]:
    if fixture_name.startswith("build"):
        return {"HTTP_X_GITLAB_EVENT": "Job Hook"}
...
def get_issue_created_event_body(payload: WildValue, include_title: bool, realm: Realm) -> str:
    ...
    return get_issue_event_message(
        user_name=get_issue_user_name(payload, realm),
        action="created",
...
def get_commented_commit_event_body(payload: WildValue, include_title: bool, realm: Realm) -> str:
    ...
    return get_commits_comment_action_message(
        get_issue_user_name(payload, realm),
        action,
...
def get_feature_flag_event_body(
    ...
    return FEATURE_FLAG_MESSAGE_TEMPLATE.format(
        user=get_issue_user_name(payload, realm),
        action=action,
...
def get_deployment_event_body(
    payload: WildValue,
    include_title: bool,
    realm: Realm,
) -> str:
    user_text = f"[{get_issue_user_name(payload, realm)}]({payload['user_url'].tame(check_string)})"
...
def get_user_mention(realm: Realm, username: str, display_name: str) -> str:
...
def get_user_name(payload: WildValue, realm: Realm) -> str:
    return get_user_mention(
        realm, payload["user_username"].tame(check_string), payload["user_name"].tame(check_string)
    )


def get_issue_user_name(payload: WildValue, realm: Realm) -> str:
    return get_user_mention(
        realm,
        payload["user"]["username"].tame(check_string),
        payload["user"]["name"].tame(check_string),
    )
