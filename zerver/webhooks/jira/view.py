...
def get_user_mention(realm: Realm, user_payload: WildValue) -> str:
    # Try to find a Zulip user by Jira username (Jira Server)
    if "name" in user_payload:
        external_account_field_name = str(DEFAULT_EXTERNAL_ACCOUNTS["jira"].name)
        jira_username = user_payload["name"].tame(check_string)
